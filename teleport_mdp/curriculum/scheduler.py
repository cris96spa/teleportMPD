"""Teleport-rate schedulers (thesis Algorithms 2 & 3).

These schedulers drive the curriculum that lowers the teleport rate ``tau`` from
its initial value ``tau_0`` down to ``0`` over training, so the agent is trained
on a sequence of teleport MDPs whose effective discount ``gamma_eff = gamma(1 - tau)``
grows back to ``gamma``. They are deliberately decoupled from Stable-Baselines3 so
the math can be unit-tested in isolation against the closed-form thesis results;
:class:`teleport_mdp.agents.teleport_ppo.TeleportPPO` only wires them to PPO.

The two model-shift helpers below are the canonical thesis formulas (confirmed
against legacy ``model_functions.compute_eps_model`` / ``compute_tau_prime`` and
thesis Theorem 5.2):

- ``eps_model = 2*gamma*tau / (n*(1 - gamma))`` is the per-update *model shift*
  budget that drives ``tau`` to ``0`` in exactly ``n`` updates.
- ``tau' = max(0, tau - eps_model*(1 - gamma)/(2*gamma))`` converts a model-shift
  budget back into a teleport-rate decrement (the inverse of the line above).

Composing the two, a static schedule decrements ``tau`` by exactly ``tau_0 / n``
each update and reaches ``0`` at update ``n``.
"""
...
def compute_tau_prime(gamma: float, tau: float, eps_model: float) -> float:
    ...
    of :func:`compute_eps_model` for a single step.
...
def compute_n_updates(gamma: float, tau: float, eps_model: float) -> int:
    ...
    the inverse of :func:`compute_eps_model`.
...
class StaticTeleportScheduler(TeleportScheduler):
    ...
    def __init__(self, gamma: float, tau_0: float, n_updates: int) -> None:
        ...
        self._eps_tau = compute_eps_model(gamma, tau_0, n_updates)
    ...
    def next_tau(self, tau: float, *, policy_shift: float | None = None) -> float:
        ...
        return compute_tau_prime(self._gamma, tau, self._eps_tau)
...
class DynamicTeleportScheduler(TeleportScheduler):
    ...
    def next_tau(self, tau: float, *, policy_shift: float | None = None) -> float:
        ...
        if eps_tau <= 0.0:
            return tau
        return compute_tau_prime(self._gamma, tau, min(eps_tau, self._eps_tau_max))
